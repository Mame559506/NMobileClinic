      ...
      <div className="card" style={{ width:"100%", maxWidth:560, maxHeight:"90vh", overflow:"auto" }} onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={onSave}>
          ...
          <div className="form-group">
            ...
            <input type="file" accept="image/*" style={{ display:"block", marginTop:4, fontSize:14 }}
          ...
          <div style={{ display:"flex", gap:24, marginBottom:20 }}>
            <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontWeight:500 }}>
              <input type="checkbox" checked={form.is_featured} onChange={set("is_featured")} /> Featured
            </label>
            <label style={{ display:"flex", alignItems:"center", gap:8, cursor:"pointer", fontWeight:500 }}>
    ...
    <div className="page">
      ...
      <div className="stats-grid" style={{ marginBottom:20 }}>
        {[
          ...
          { label:"Low Stock", value:lowStockCount, color:"warning", f:"low" },
          { label:"Out of Stock", value:outOfStockCount, color:"danger", f:"out" },
        ].map(s => (
      ...
      {lowStockCount > 0 && (
        <div style={{ background:"rgba(248,150,30,0.1)", border:"1px solid var(--warning)", borderRadius:8, padding:"12px 16px", marginBottom:20, display:"flex", alignItems:"center", gap:10 }}>
          ...
          <span style={{ color:"var(--warning)", fontWeight:500 }}>{lowStockCount} product(s) running low on stock.</span>
        </div>
      )}
      ...
      <div className="table-container">
        <div className="table-responsive">
          <table className="data-table">
            <thead>
              <tr><th>Image</th><th>Product</th><th>Category</th><th>Price (ETB)</th><th>Stock</th><th>Status</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr key={p.id}>
                  ...
                  <td><div style={{ fontWeight:600 }}>{p.name}</div>{p.brand && <div style={{ fontSize:12, color:"var(--gray)" }}>{p.brand}</div>}</td>
                  <td>{p.category_name || "-"}</td>
                  <td style={{ fontWeight:600, color:"var(--primary)" }}>ETB {parseFloat(p.price).toFixed(2)}</td>
                  <td>{stockBadge(p.stock_quantity)}</td>
                  <td><span className={"status-badge " + (p.is_active ? "status-completed" : "status-cancelled")}>{p.is_active ? "Active" : "Inactive"}</span></td>
                  <td>
                    <div style={{ display:"flex", gap:6 }}>
                      <button className="btn" style={{ padding:"6px 10px", fontSize:12 }} onClick={() => openEdit(p)}><FaEdit /></button>
                      <button className={"btn " + (p.is_active ? "btn-outline" : "btn-success")} style={{ padding:"6px 10px", fontSize:12 }} onClick={() => handleToggleActive(p)}>{p.is_active ? "Off" : "On"}</button>
                      <button className="btn btn-danger" style={{ padding:"6px 10px", fontSize:12 }} onClick={() => handleDelete(p.id)}><FaTrash /></button>
                    </div>
                  </td>
                </tr>
              ))}
